    ...
    >
      <div className="container mt-5 pt-5">
        ...
        {!loading && items.length === 0 && (
          <div className="card shadow-lg border-0 rounded-3 mb-4" style={{ backgroundColor: '#fff' }}>
            <div className="card-body py-4">
        ...
        {items.map(item => {
          const isOccupied = String(item.availabilityStatus || "").toLowerCase() !== "available";
          ...
          return (
            ...
            >
              <div className="card-body p-3">
                {item.imageSrc && (
                  <img src={item.imageSrc} alt={item.propertyName} style={imageStyle} />
                )}
                <div className="mt-3">
                  <p><strong>Owner name:</strong> {item.ownerName}</p>
                  <p><strong>Property name:</strong> {item.propertyName}</p>
                  <p><strong>Amount:</strong> ₹{item.rentAmount}</p>
        ...
        {total > 0 && (
          <div className="mt-4 card shadow-lg border-0 rounded-3" style={{ backgroundColor: '#fff' }}>
            <div className="card-body p-3">
              <h4>Total: ₹{total}</h4>
              <p className="text-muted">
                Proceed to lease each property individually using the "Lease it" button above.
              </p>
            </div>
          </div>
        )}
